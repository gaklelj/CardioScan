...
const translations = {
  en: {
    ...
    errorInference: 'ECG analysis failed.',
    ecgAwaitingAnalysis: 'The result will appear automatically after 10 seconds of ECG data and update during recording. The questionnaire is optional for ECG analysis.',
    ecgLastAnalysis: 'Last analyzed position in the recording',
    ecgSecondsUnit: 's',
    ecgOpenQuestionnaire: 'Fill in questionnaire',
    ecgErrorTooShort: 'At least 10 seconds of ECG data are required. Record for 12–15 seconds and try again. Your recording is saved in history.',
    ecgErrorGaps: 'The recording contains missing samples. Check the connection and repeat the recording. Your recording is saved in history.',
    ecgErrorModel: 'The local ECG model is unavailable. Check the backend startup and model files. No API key is required.',
    ecgErrorTiming: 'Sample timing is missing or invalid. Check the firmware and repeat the recording.',
    ecgErrorConnection: 'Cannot reach the local analysis service. Check that the backend is running on port 6767.',
    result: 'Result',
    detectedN: 'Detected:',
  ...
  ru: {
    ...
    errorInference: 'Не удалось проанализировать ЭКГ.',
    ecgAwaitingAnalysis: 'Результат появится автоматически после накопления 10 секунд ЭКГ и будет обновляться во время записи. Для анализа ЭКГ опросник не требуется.',
    ecgLastAnalysis: 'Последний анализ — позиция в записи',
    ecgSecondsUnit: 'с',
    ecgOpenQuestionnaire: 'Заполнить опросник',
    ecgErrorTooShort: 'Для анализа нужно минимум 10 секунд данных ЭКГ. Запишите 12–15 секунд и повторите анализ. Запись сохранена в истории.',
    ecgErrorGaps: 'В записи есть пропущенные отсчёты. Проверьте соединение и повторите запись. Запись сохранена в истории.',
    ecgErrorModel: 'Локальная модель ЭКГ недоступна. Проверьте запуск сервера и файлы модели. API-ключ не требуется.',
    ecgErrorTiming: 'Нет корректных временных меток или частоты измерений. Проверьте прошивку и повторите запись.',
    ecgErrorConnection: 'Нет связи с локальным сервисом анализа. Проверьте, что сервер запущен на порту 6767.',
    result: 'Результат',
  ...
  kz: {
    ...
    errorInference: 'ЭКГ талдауы орындалмады.',
    ecgAwaitingAnalysis: 'Нәтиже 10 секунд ЭКГ жиналғаннан кейін автоматты түрде пайда болып, жазу кезінде жаңарады. ЭКГ талдауы үшін сауалнама қажет емес.',
    ecgLastAnalysis: 'Соңғы талдау — жазбадағы уақыт',
    ecgSecondsUnit: 'с',
    ecgOpenQuestionnaire: 'Сауалнаманы толтыру',
    ecgErrorTooShort: 'Талдауға кемінде 10 секунд ЭКГ деректері қажет. 12–15 секунд жазып, қайталаңыз. Жазба тарихта сақталды.',
    ecgErrorGaps: 'Жазбада өткізіп алған өлшемдер бар. Қосылымды тексеріп, қайта жазыңыз. Жазба тарихта сақталды.',
    ecgErrorModel: 'Жергілікті ЭКГ моделі қолжетімсіз. Сервер мен модель файлдарын тексеріңіз. API кілті қажет емес.',
    ecgErrorTiming: 'Уақыт белгілері немесе өлшеу жиілігі жарамсыз. Микробағдарламаны тексеріп, қайта жазыңыз.',
    ecgErrorConnection: 'Жергілікті талдау қызметімен байланыс жоқ. Сервердің 6767 портында іске қосылғанын тексеріңіз.',
    result: 'Нәтиже',
    detectedN: 'Анықталды:',
